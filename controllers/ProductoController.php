...
class ProductoController {
    ...
    public function store() {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $this->productoModel->createProducto($nombre, $descripcion, $precio, $stock, $fecha_registro, $image_path);
    ...
    public function edit($id) {
        $producto = $this->productoModel->getProductoById($id);
        // Cargar categorías y proveedores para mostrarlos en el formulario de edición (si los necesitas)
    ...
    public function update($id) {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (isset($_FILES['imagen']) && $_FILES['imagen']['error'] == 0) {
                $imagen = $_FILES['imagen'];
                $target_dir = "../subido/"; // Ajuste en la ruta de la imagen
                $image_path = $target_dir . basename($imagen["name"]);
            ...
            $this->productoModel->updateProducto($id, $nombre, $descripcion, $precio, $stock, $image_path);
            header("Location: ProductoController.php?action=index");
        }
    }
